...
export function Dashboard() {
  ...
  return (
    <div className="p-8 space-y-8 pb-24 md:pb-8 relative">
      {/* Top Header & Filters */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="bg-primary hover:bg-blue-600 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-lg shadow-primary/20 whitespace-nowrap"
          >
            + Add Transaction
          </button>
        </div>
      ...
      {showIncomeReminder && (
        <motion.div 
          ...
          className="bg-gradient-to-r from-amber-500/10 to-amber-600/10 border border-amber-500/20 rounded-xl p-4 flex items-start sm:items-center gap-4"
        >
          <div className="bg-amber-500/20 p-2 rounded-full text-amber-500 shrink-0">
            <AlertCircle size={24} />
          </div>
          <div className="flex-1">
            <h4 className="text-amber-500 font-medium">It's a new month!</h4>
            <p className="text-slate-300 text-sm mt-0.5">You haven't logged any income for {new Date(selectedMonth).toLocaleString('default', { month: 'long' })} yet. Add your salary to keep your balance accurate.</p>
          </div>
          <button 
            onClick={() => setIsFormOpen(true)}
        ...
        {cards.map((card, i) => (
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <p className="text-3xl font-bold text-slate-100 mt-2">{card.amount}</p>
              <div className="flex items-center space-x-2 mt-4">
                <span className={`text-sm font-medium flex items-center ${card.isText ? 'text-slate-400' : (card.isUp ? 'text-success' : 'text-danger')}`}>
                  {!card.isText && (card.isUp ? <ArrowUpRight size={16} className="mr-1" /> : <ArrowDownRight size={16} className="mr-1" />)}
                  {card.trend}
                </span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Line Chart */}
        <div className="lg:col-span-2 bg-surface border border-border rounded-2xl p-6 h-[400px] flex flex-col">
          <h3 className="text-lg font-semibold text-slate-200 mb-4">Yearly Trends ({selectedMonth.split('-')[0]})</h3>
        ...
        <div className="bg-surface border border-border rounded-2xl p-6 h-[400px] flex flex-col">
          <h3 className="text-lg font-semibold text-slate-200 mb-4">Monthly Spending Breakdown</h3>
          <div className="flex-1 min-h-0">
            {categoryData.length > 0 ? (
              <CategoryDonutChart data={categoryData} />
            ) : (
              <div className="h-full flex items-center justify-center text-slate-500">No expenses this month</div>
            )}
          </div>
        </div>
      </div>
